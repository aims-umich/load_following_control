...
num_drums =2; % select the number of drums 8,4,2,1 
...
function [Rho_d0, Reactivity_per_degree, u0, W] = number_drums(num_drums)
    switch num_drums
        case 8
            Rho_d0 = -0.033085599;
...
min = 0;
max_rate = 0.5;
[Rho_d0, Reactivity_per_degree, u0, W] = number_drums(num_drums);
...
stcController(0, 0, 0, lambda, W, T_C, max, min, max_rate, 1, u0);
...
for i = 1:nt - 1
    [dx, rho(i)] = reactorDAE(0, x_stc(i,:), u_stc(i), Rho_d0, Reactivity_per_degree, Xe0, I0, Pi,num_drums);
    x_stc(i + 1, :) = x_stc(i, :) + dx' * dt;
    u_stc(i + 1) = stcController(time(i), x_stc(i + 1, 1), ref(i + 1), lambda, W, T_C, max, min, max_rate, 0, 0);
end

[~, rho(nt)] = reactorDAE(0, x_stc(nt, :), u_stc(nt), Rho_d0, Reactivity_per_degree, Xe0, I0, Pi,num_drums);
t = time;

u_stc = u_stc(:);

du_stc = [0; diff(u_stc)/dt];


% du_stc = zeros(size(u_stc));  
% du_stc(1:2) = 0.1;        
% du_stc(3:end) = diff(u_stc(2:end)) / dt; 
% control_effort_withoutFilter=norm(du_stc);
% fprintf('control_effort_withoutFilter: %f\n', control_effort_withoutFilter);
% 
% 
% window_size = 200;  % Adjust this window size to control the degree of smoothing
% du_stc = movmean(du_stc, window_size); 



control_effort = norm(du_stc);
...
du_stc_smooth = du_stc;
for i = window_size + 1:length(du_stc) - window_size
    du_stc_smooth(i) = mean(du_stc(i - window_size:i + window_size));
end
...
% Display results
%fprintf('Mean Absolute Error (MAE): %.8f\n', MAE);
...
t=t/60;
subplot(4,2,[1,2]);
plot(t, x_stc(:,1) * 100, 'LineWidth', 2);
hold on;
...
title('STC microreactor control system core power simulation with 4 control drums', 'FontSize', font_size, 'FontWeight', font_weight);
ylim([0, 120]);
xlim([0,100])
...
subplot(4,2,4);
plot(t(100:end-100), du_stc_smooth(100:end-100), 'LineWidth', 2);
grid on;
xlabel('Time (min)', 'FontSize', font_size, 'FontWeight', font_weight);
...
subplot(4,2,7);
plot(t, x_stc(:, 11), 'LineWidth', 2);
%plot(t(700:end), x(700:end, 11), 'LineWidth', 2);
grid on;
xlabel('Time (min)', 'FontSize', font_size, 'FontWeight', font_weight);
ylabel('Temperature (K)', 'FontSize', font_size, 'FontWeight', font_weight);
...
subplot(4,2,8);
plot(t, x_stc(:,12), 'LineWidth', 2);
%plot(t(700:end), x(700:end, 12), 'LineWidth', 2);
grid on;
xlabel('Time (min)', 'FontSize', font_size, 'FontWeight', font_weight);
ylabel('Temperature (K)', 'FontSize', font_size, 'FontWeight', font_weight);
...
saveas(gcf, 'STC_power_simulation.png');
...
function [dx, rho] = reactorDAE(t, x, u, Rho_d0, Reactivity_per_degree,Xe0, I0, Pi,num_drums)
    ...
    %% ODEs
    dx      = zeros(12, 1);
    %rho     = Rho_d1 + alpha_f * (Tf - Tf0) + alpha_c * (Tc - Tc0) + alpha_m * (Tm - Tm0) - Sig_x * (X - Xe0) / Sum_f; % 8,2,1 drums
...
% Supertwisting controller
function u = stcController(t, measurement, setpoint, lambda, W, T_C, max, min, max_rate, init, u0)
    persistent w err_prev deriv_prev t_prev command_sat_prev command_prev;
    ...
    s = err + deriv_filt;
    w = w + W * sign(s) * T;
    command = lambda * sqrt(abs(s)) * sign(s) + w;
